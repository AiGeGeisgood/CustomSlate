...
DECLARE_CYCLE_STAT(TEXT("SMyTextBlock::SetText Time"), Stat_SlateTextBlockSetText, STATGROUP_SlateVerbose)
DECLARE_CYCLE_STAT(TEXT("SMyTextBlock::OnPaint Time"), Stat_SlateTextBlockOnPaint, STATGROUP_SlateVerbose)
DECLARE_CYCLE_STAT(TEXT("SMyTextBlock::ComputeDesiredSize"), Stat_SlateTextBlockCDS, STATGROUP_SlateVerbose)
DECLARE_CYCLE_STAT(TEXT("SMyTextBlock::ComputeVolitility"), Stat_SlateTextBlockCV, STATGROUP_SlateVerbose)

SMyTextBlock::SMyTextBlock()
{
	SetCanTick(false);
...
void SMyTextBlock::Construct( const FArguments& InArgs )
{
...
void SMyTextBlock::SetText( const TAttribute< FString >& InText )
{
...
void SMyTextBlock::SetText( const FString& InText )
{
	SetText(FText::AsCultureInvariant(InText));
}

void SMyTextBlock::SetText( const TAttribute< FText >& InText )
{
	if (InText.IsSet() && !InText.IsBound())
	{
		SetText(InText.Get());
...
void SMyTextBlock::SetText( const FText& InText )
{
	SCOPE_CYCLE_COUNTER(Stat_SlateTextBlockSetText);
...
void SMyTextBlock::SetHighlightText(TAttribute<FText> InText)
{
	HighlightText = InText;
}

int32 SMyTextBlock::OnPaint( const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const
{
	...
	else
	{
		...
		{
			const_cast<SMyTextBlock*>(this)->Invalidate(EInvalidateWidget::Layout);
		}
	}
...
FVector2D SMyTextBlock::ComputeDesiredSize(float LayoutScaleMultiplier) const
{
...
bool SMyTextBlock::ComputeVolatility() const
{
...
void SMyTextBlock::SetFont(const TAttribute< FSlateFontInfo >& InFont)
{
...
void SMyTextBlock::SetStrikeBrush(const TAttribute<const FSlateBrush*>& InStrikeBrush)
{
...
void SMyTextBlock::SetColorAndOpacity(const TAttribute<FSlateColor>& InColorAndOpacity)
{
...
void SMyTextBlock::SetTextStyle(const FTextBlockStyle* InTextStyle)
{
	if (InTextStyle)
	{
...
void SMyTextBlock::SetTextShapingMethod(const TOptional<ETextShapingMethod>& InTextShapingMethod)
{
	if (!bSimpleTextMode)
	{
...
void SMyTextBlock::SetTextFlowDirection(const TOptional<ETextFlowDirection>& InTextFlowDirection)
{
	if(!bSimpleTextMode)
	{
...
void SMyTextBlock::SetWrapTextAt(const TAttribute<float>& InWrapTextAt)
{
...
void SMyTextBlock::SetAutoWrapText(const TAttribute<bool>& InAutoWrapText)
{
...
void SMyTextBlock::SetWrappingPolicy(const TAttribute<ETextWrappingPolicy>& InWrappingPolicy)
{
...
void SMyTextBlock::SetShadowOffset(const TAttribute<FVector2D>& InShadowOffset)
{
...
void SMyTextBlock::SetShadowColorAndOpacity(const TAttribute<FLinearColor>& InShadowColorAndOpacity)
{
...
void SMyTextBlock::SetMinDesiredWidth(const TAttribute<float>& InMinDesiredWidth)
{
...
void SMyTextBlock::SetLineHeightPercentage(const TAttribute<float>& InLineHeightPercentage)
{
...
void SMyTextBlock::SetMargin(const TAttribute<FMargin>& InMargin)
{
...
void SMyTextBlock::SetJustification(const TAttribute<ETextJustify::Type>& InJustification)
{
...
FTextBlockStyle SMyTextBlock::GetComputedTextStyle() const
{
...
#if WITH_ACCESSIBILITY
TSharedRef<FSlateAccessibleWidget> SMyTextBlock::CreateAccessibleWidget()
{
	return MakeShareable<FSlateAccessibleWidget>(new FSlateAccessibleTextBlock(SharedThis(this)));
}

TOptional<FText> SMyTextBlock::GetDefaultAccessibleText(EAccessibleType AccessibleType) const
{
	return GetText();
}
#endif
