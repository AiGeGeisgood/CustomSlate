...
void SMyOverlay::Construct( const SMyOverlay::FArguments& InArgs )
{
...
void SMyOverlay::OnArrangeChildren( const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren ) const
{
...
FVector2D SMyOverlay::ComputeDesiredSize( float ) const
{
...
FChildren* SMyOverlay::GetChildren()
{
	return &Children;
}

int32 SMyOverlay::OnPaint( const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const
{
...
SMyOverlay::FOverlaySlot& SMyOverlay::AddSlot( int32 ZOrder )
{
...
void SMyOverlay::RemoveSlot( int32 ZOrder )
{
	if (ZOrder != INDEX_NONE)
	{
		for( int32 ChildIndex=0; ChildIndex < Children.Num(); ++ChildIndex )
...
void SMyOverlay::ClearChildren()
{
	Children.Empty();
	Invalidate(EInvalidateWidget::Layout);
}

int32 SMyOverlay::GetNumWidgets() const
{
	return Children.Num();
}

bool SMyOverlay::RemoveSlot( TSharedRef< SWidget > Widget )
{
	// Search and remove
