...
void SMyButton::Construct( const FArguments& InArgs )
{
	...
	// Only do this if we're exactly an SMyButton
...
int32 SMyButton::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
...
void SMyButton::OnMouseCaptureLost(const FCaptureLostEvent& CaptureLostEvent)
{
	Release();
}

FReply SMyButton::ExecuteOnClick()
{
...
bool SMyButton::ComputeVolatility() const
{
...
TEnumAsByte<EButtonClickMethod::Type> SMyButton::GetClickMethodFromInputType(const FPointerEvent& MouseEvent) const
{
...
void SMyButton::SetClickMethod(EButtonClickMethod::Type InClickMethod)
{
	ClickMethod = InClickMethod;
}

void SMyButton::SetTouchMethod(EButtonTouchMethod::Type InTouchMethod)
{
	TouchMethod = InTouchMethod;
}

void SMyButton::SetPressMethod(EButtonPressMethod::Type InPressMethod)
{
	PressMethod = InPressMethod;
}

#if WITH_ACCESSIBILITY
TSharedRef<FSlateAccessibleWidget> SMyButton::CreateAccessibleWidget()
{
