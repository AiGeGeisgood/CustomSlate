...
void SMySizeBox::Construct( const FArguments& InArgs )
{
...
void SMySizeBox::SetContent(const TSharedRef< SWidget >& InContent)
{
	ChildSlot
		[
			InContent
...
void SMySizeBox::SetHAlign(EHorizontalAlignment HAlign)
{
...
void SMySizeBox::SetVAlign(EVerticalAlignment VAlign)
{
	// TODO SlateGI - Fix Slots
...
void SMySizeBox::SetPadding(const TAttribute<FMargin>& InPadding)
{
	// TODO SlateGI - Fix Slots
...
void SMySizeBox::SetWidthOverride(TAttribute<FOptionalSize> InWidthOverride)
{
	SetAttribute(WidthOverride, InWidthOverride, EInvalidateWidgetReason::Layout);
}

void SMySizeBox::SetHeightOverride(TAttribute<FOptionalSize> InHeightOverride)
{
	SetAttribute(HeightOverride, InHeightOverride, EInvalidateWidgetReason::Layout);
}

void SMySizeBox::SetMinDesiredWidth(TAttribute<FOptionalSize> InMinDesiredWidth)
{
	SetAttribute(MinDesiredWidth, InMinDesiredWidth, EInvalidateWidgetReason::Layout);
}

void SMySizeBox::SetMinDesiredHeight(TAttribute<FOptionalSize> InMinDesiredHeight)
{
	SetAttribute(MinDesiredHeight, InMinDesiredHeight, EInvalidateWidgetReason::Layout);
}

void SMySizeBox::SetMaxDesiredWidth(TAttribute<FOptionalSize> InMaxDesiredWidth)
{
	SetAttribute(MaxDesiredWidth, InMaxDesiredWidth, EInvalidateWidgetReason::Layout);
}

void SMySizeBox::SetMaxDesiredHeight(TAttribute<FOptionalSize> InMaxDesiredHeight)
{
	SetAttribute(MaxDesiredHeight, InMaxDesiredHeight, EInvalidateWidgetReason::Layout);
}

void SMySizeBox::SetMinAspectRatio(TAttribute<FOptionalSize> InMinAspectRatio)
{
...
void SMySizeBox::SetMaxAspectRatio(TAttribute<FOptionalSize> InMaxAspectRatio)
{
	SetAttribute(MaxAspectRatio, InMaxAspectRatio, EInvalidateWidgetReason::Layout);
}

FVector2D SMySizeBox::ComputeDesiredSize( float ) const
{
...
float SMySizeBox::ComputeDesiredWidth() const
{
...
float SMySizeBox::ComputeDesiredHeight() const
{
	// If the user specified a fixed width or height, those values override the Box's content.
	const FVector2D& UnmodifiedChildDesiredSize = ChildSlot.GetWidget()->GetDesiredSize() + ChildSlot.SlotPadding.Get().GetDesiredSize();
...
void SMySizeBox::OnArrangeChildren( const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren ) const
{
...
FChildren* SMySizeBox::GetChildren()
{
	return &ChildSlot;
}

int32 SMySizeBox::OnPaint( const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const
{
	// An SMySizeBox just draws its only child
