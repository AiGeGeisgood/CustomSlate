 ...
 */
class CUSTOMWIDGET_API SMyScaleBox : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SMyScaleBox)
	: _Content()
	, _HAlign(HAlign_Center)
	, _VAlign(VAlign_Center)
	, _StretchDirection(EMyStretchDirection::Both)
	, _Stretch(EMyStretch::None)
	...
	{}
		...
		SLATE_ATTRIBUTE(EMyStretchDirection::Type, StretchDirection)
		
		/** The stretching rule to apply when content is stretched */
		SLATE_ATTRIBUTE(EMyStretch::Type, Stretch)
...
protected:
	...
	TAttribute<EMyStretchDirection::Type> StretchDirection;

	/** The method of scaling that is applied to the content. */
	TAttribute<EMyStretch::Type> Stretch;
