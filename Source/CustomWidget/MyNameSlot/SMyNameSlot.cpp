...
void SMyNameSlot::Construct( const FArguments& InArgs )
{
...
void SMyNameSlot::SetContent(const TSharedRef< SWidget >& InContent)
{
	ChildSlot
		[
			InContent
...
void SMyNameSlot::SetHAlign(EHorizontalAlignment HAlign)
{
...
void SMyNameSlot::SetVAlign(EVerticalAlignment VAlign)
{
	// TODO SlateGI - Fix Slots
...
void SMyNameSlot::SetPadding(const TAttribute<FMargin>& InPadding)
{
	// TODO SlateGI - Fix Slots
...
void SMyNameSlot::SetWidthOverride(TAttribute<FOptionalSize> InWidthOverride)
{
	SetAttribute(WidthOverride, InWidthOverride, EInvalidateWidgetReason::Layout);
}

void SMyNameSlot::SetHeightOverride(TAttribute<FOptionalSize> InHeightOverride)
{
	SetAttribute(HeightOverride, InHeightOverride, EInvalidateWidgetReason::Layout);
}

void SMyNameSlot::SetMinDesiredWidth(TAttribute<FOptionalSize> InMinDesiredWidth)
{
	SetAttribute(MinDesiredWidth, InMinDesiredWidth, EInvalidateWidgetReason::Layout);
}

void SMyNameSlot::SetMinDesiredHeight(TAttribute<FOptionalSize> InMinDesiredHeight)
{
	SetAttribute(MinDesiredHeight, InMinDesiredHeight, EInvalidateWidgetReason::Layout);
}

void SMyNameSlot::SetMaxDesiredWidth(TAttribute<FOptionalSize> InMaxDesiredWidth)
{
	SetAttribute(MaxDesiredWidth, InMaxDesiredWidth, EInvalidateWidgetReason::Layout);
}

void SMyNameSlot::SetMaxDesiredHeight(TAttribute<FOptionalSize> InMaxDesiredHeight)
{
	SetAttribute(MaxDesiredHeight, InMaxDesiredHeight, EInvalidateWidgetReason::Layout);
}

void SMyNameSlot::SetMinAspectRatio(TAttribute<FOptionalSize> InMinAspectRatio)
{
...
void SMyNameSlot::SetMaxAspectRatio(TAttribute<FOptionalSize> InMaxAspectRatio)
{
	SetAttribute(MaxAspectRatio, InMaxAspectRatio, EInvalidateWidgetReason::Layout);
}

FVector2D SMyNameSlot::ComputeDesiredSize( float ) const
{
...
float SMyNameSlot::ComputeDesiredWidth() const
{
...
float SMyNameSlot::ComputeDesiredHeight() const
{
	// If the user specified a fixed width or height, those values override the Box's content.
	const FVector2D& UnmodifiedChildDesiredSize = ChildSlot.GetWidget()->GetDesiredSize() + ChildSlot.SlotPadding.Get().GetDesiredSize();
...
void SMyNameSlot::OnArrangeChildren( const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren ) const
{
...
FChildren* SMyNameSlot::GetChildren()
{
	return &ChildSlot;
}

int32 SMyNameSlot::OnPaint( const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const
{
	// An SMyNameSlot just draws its only child
